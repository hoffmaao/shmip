...
"""SHMIP Suite D: seasonal forcing figure (T, melt, N bands, Q).

Usage: cd shmip_D && python plot_shmip.py
Output: outputs/shmip_D.png
"""
...
CSV_DIR = "outputs/csv"
OUTFILE = "outputs/shmip_D.png"

Lx = 100e3
CASES   = ["D1", "D2", "D3", "D4", "D5"]
D_MAP   = {"D1": -4.0, "D2": -2.0, "D3": 0.0, "D4": +2.0, "D5": +4.0}
...
S_MID = 6 * (np.sqrt(50e3 + 5000) - np.sqrt(5000)) + 1

BANDS = [
    ("lower (0-25 km)",   0.0,   25e3),
    ("middle (25-75 km)", 25e3,  75e3),
    ("upper (75-100 km)", 75e3, 100e3),
]

CASE_COLORS = {
    "D1": "#2166ac", "D2": "#74add1", "D3": "#878787",
    "D4": "#f46d43", "D5": "#a50026",
}
...
if not csv_data:
    sys.exit("No CSV data found — run run_shmip_D.py first.")
...
for tag in CASES:
    ...
    ax_melt.plot(t_cont, m, lw=2.0, color=CASE_COLORS[tag],
                 label=f"{tag}")
...
fig.suptitle(
    "SHMIP Suite D — Seasonal forcing\n"
    "D1-D5: dT = -4, -2, 0, +2, +4 C",
    fontsize=12,
)
